...
import {newDefenseAction,newDefenseDraft,normalizeDefenseMetadata,serializeDefenseDraft,defenseMissingFields,defenseActionLabel,defenseActionStatus,defenseCardStatuses,defenseOverallTone,defenseThrowTLU,defenseOfficialText,defenseOfficialRecommendation} from '../js/defense.js';
...
const editorContext={
  ...
  defenseActionStatus,defenseMissingFields,defenseActionLabel,defenseOfficialText,defenseOfficialRecommendation,
...
for(const name of ['defensePositionOptions','defenseChoiceButtons','defenseFieldGroup','defenseActionSummary','defenseActionChooserHtml','defenseFieldDetailOptions','defenseOptionalDetailsHtml','defenseJudgmentHtml','defenseActionFieldsHtml','defenseActionCardHtml','defenseSituationHtml','defenseOutcomeHtml','defenseEditorHtml','defenseJudgmentDetailRows'])vm.runInNewContext(extractFunction(app,name),editorContext);
...
for(const [action,token] of [[field,'타구 도달'],[firstThrow,'송구 타이밍'],[receive,'들어온 송구'],[newDefenseAction('cover','cover-test'),'수비 역할']])assert.match(editorContext.defenseActionFieldsHtml('create',action),new RegExp(token));
...
const firstChooser=editorContext.defenseActionChooserHtml('create',-1);for(const type of ['field','receive','throw','cover'])assert.match(firstChooser,new RegExp(`data-defense-add-type="${type}"`));
...
const filterContext={
  ui:{historyStatus:'all',historyDefenseStatus:'all',historyDefenseAction:'all',historyFieldResult:'all',historyThrowResult:'all',historyReceiveResult:'all',historyOfficial:'all'},
...
assert.match(css,/sequential defense play editor/);assert.match(css,/\.defense-choice-grid\{display:grid;grid-template-columns:repeat\(4,minmax\(0,1fr\)\)/);assert.match(css,/@media\(max-width:780px\)\{\.defense-choice-grid[\s\S]*?repeat\(3,minmax\(0,1fr\)\)[\s\S]*?\.defense-action-type-grid\{grid-template-columns:1fr 1fr\}/);assert.match(css,/@media\(max-width:599px\)\{[\s\S]*?\.defense-context-card[\s\S]*?\.defense-choice-grid[\s\S]*?grid-template-columns:1fr 1fr/);assert.match(css,/@media\(pointer:coarse\)\{\.defense-choice[\s\S]*?min-height:48px/);assert.match(css,/\.defense-edit-actions\{position:sticky/);

assert.equal(read('VERSION').trim(),'7.5.0');assert.match(app,/appVersion:'7\.5\.0'/);assert.match(sw,/baseball-diary-v7\.5\.0/);assert.match(sw,/\.\/js\/defense\.js\?v=7\.5\.0/);assert.match(html,/js\/app\.js\?v=7\.5\.0/);assert.doesNotMatch([app,html,sw].join('\n'),/[?&]v=7\.4\.2/,'실행 파일에 이전 V7.4.2 캐시 쿼리가 남으면 안 됩니다.');
console.log('V7.5.0 defense smoke tests: PASS');
