...
assert.equal(read('VERSION').trim(),'7.6.1');assert.match(app,/appVersion:'7\.6\.1'/);assert.match(sw,/baseball-diary-v7\.6\.1/);assert.match(sw,/\.\/js\/defense\.js\?v=7\.6\.1/);assert.match(html,/js\/app\.js\?v=7\.6\.1/);assert.doesNotMatch([app,html,sw].join('\n'),/[?&]v=7\.4\.2/,'실행 파일에 이전 V7.4.2 캐시 쿼리가 남으면 안 됩니다.');
console.log('V7.5 defense regression smoke tests on V7.6.1: PASS');
