...
assert.equal(read('VERSION').trim(),'7.9.1');assert.match(app,/appVersion:'7\.9\.1'/);assert.match(html,/야구일기 V7\.9\.1/);assert.match(sw,/baseball-diary-v7\.9\.1/);
assert.doesNotMatch([app,analytics,analysisScope,html,sw].join('\n'),/[?&]v=7\.6\.0/,'실행 파일에 이전 캐시 쿼리가 남으면 안 됩니다.');

console.log('V7.9.1 OPS and UI consistency smoke tests: PASS');
