...
assert.equal(read('VERSION').trim(),'7.6.1');
assert.match(app,/appVersion:'7\.6\.1'/);
assert.match(sw,/baseball-diary-v7\.6\.1/);
assert.match(html,/js\/app\.js\?v=7\.6\.1/);
assert.doesNotMatch([app,analysisScope,html,sw].join('\n'),/[?&]v=7\.3\.0/,'실행 파일에 V7.3 캐시 쿼리가 남으면 안 됩니다.');

console.log('V7.4 regression smoke tests on V7.6.1: PASS');
