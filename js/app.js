...
function renderInputSummary(){
  ...
  if(ui.inputMode==='game'&&ui.domain==='pitching'){
    ...
    const s=defenseSummary(data,{athleteId:a.id,date:ui.inputDate});kicker='DEFENSE';compact=[['PLAYS',s.plays],['SECURE',pct(s.securePct,0)],['THROWS',s.throwAttempts],['THROW TLU',n2(s.throwTLU)]];details=`<div class="big-stat"><strong>${s.plays}</strong><span>DEFENSIVE PLAYS</span></div>${perfRows([['REACH',pct(s.reachPct,0)],['CLEAN',pct(s.cleanPct,0)],['SECURE',pct(s.securePct,0)],['ON-TARGET',pct(s.onTargetPct,0)],['CATCHABLE',pct(s.catchablePct,0)],['RECEIVE',pct(s.receivePct,0)],['DECISION',pct(s.judgmentAppropriatePct,0)],['수비 송구 TLU',n2(s.throwTLU)]])}`;
...
function ensureDefenseCreateDraft(){
  ...
  if(!defenseCreateDraft||defenseCreateDraft._context!==context){const a=athlete(),side=a?.throws==='S'?null:(a?.throws||null);defenseCreateDraft=newDefenseDraft({position:defaultDefensePosition(),throwSide:side});defenseCreateDraft._context=context;resetDefenseEditorState('create');}
...
function defenseActionSummary(action){
  ...
  if(action.type==='field'){if(action.battedBall)parts.push(action.battedBall);if(action.difficulty)parts.push({routine:'쉬움',normal:'보통',difficult:'어려움'}[action.difficulty]);}
  if(action.type==='throw'){if(action.target)parts.push(throwTargetLabel(action.target)||action.target);if(action.timing)parts.push({on_time:'제시간',late:'늦음',no_chance:'아웃 기회 없음'}[action.timing]);}
  if(action.type==='receive'){if(action.target)parts.push(throwTargetLabel(action.target)||action.target);if(action.incoming)parts.push({on_target:'정확',high:'높음',low:'낮음',wide:'벗어남',bounce:'바운드',uncatchable:'받기 불가'}[action.incoming]);}
...
function defenseFieldDetailOptions(position){return ['LF','CF','RF'].includes(position)?[['','미입력'],['FORWARD','앞으로'],['STRAIGHT','정면'],['LATERAL','좌우'],['BACK','뒤로']]:[['','미입력'],['FRONT','정면'],['FOREHAND','포핸드'],['BACKHAND','백핸드'],['CHARGE','전진']];}
function defenseOptionalDetailsHtml(mode,action){
  const state=defenseEditorState[mode],open=state.detailsOpen.has(action.id);
  let body='';
  if(action.type==='field')body=defenseFieldGroup('처리 형태 · 선택',defenseChoiceButtons(mode,action,'fieldingType',defenseFieldDetailOptions(defenseDraft(mode).position),action.fieldingType))+defenseFieldGroup('방향 · 선택',defenseChoiceButtons(mode,action,'direction',[['','미입력'],['L','좌'],['C','중'],['R','우']],action.direction));
  else if(action.type==='throw')body=`<div class="form-grid defense-extra-grid"><label>거리 m · 선택<input type="number" min="0" data-defense-action-input="distance" data-defense-action-id="${action.id}" value="${action.distance??''}" /></label><label>구속 km/h · 선택<input type="number" min="0" max="200" data-defense-action-input="velocity" data-defense-action-id="${action.id}" value="${action.velocity??''}" /></label></div>`;
  else if(action.type==='receive'){const techniques=[['','미입력'],['normal','일반'],['stretch','스트레치'],['scoop','스쿱'],['block','블로킹']];if(action.technique==='tag')techniques.push(['tag','태그 · 기존 방식']);if(action.technique==='base_hold')techniques.push(['base_hold','베이스 유지 · 기존 방식']);body=defenseFieldGroup('받는 기술 · 선택',defenseChoiceButtons(mode,action,'technique',techniques,action.technique));}
  else if(action.type==='tag')body=defenseFieldGroup('태그 대상 · 선택',defenseChoiceButtons(mode,action,'targetRunner',[['','미입력'],['BR','타자주자'],['R1','1루 주자'],['R2','2루 주자'],['R3','3루 주자'],['UNKNOWN','대상 모름']],action.targetRunner));
  else if(action.type==='base')body=defenseFieldGroup('플레이 목적 · 선택',defenseChoiceButtons(mode,action,'purpose',[['','미입력'],['force','포스 플레이'],['appeal','어필 플레이'],['other','기타']],action.purpose));
  else body='<p class="defense-helper">커버 위치와 의사소통에 관한 세부 평가는 아래 판단 평가에서 기록할 수 있습니다.</p>';
...
function defenseActionFieldsHtml(mode,action){
  if(action.type==='field')return defenseFieldGroup('타구',defenseChoiceButtons(mode,action,'battedBall',[['','미입력'],['GB','GB','땅볼'],['LD','LD','라인드라이브'],['FB','FB','뜬공'],['PU','PU','내야 뜬공'],['BUNT','번트']],action.battedBall))+defenseFieldGroup('처리 난도',defenseChoiceButtons(mode,action,'difficulty',[['','미입력'],['routine','쉬움'],['normal','보통'],['difficult','어려움']],action.difficulty))+defenseFieldGroup('타구 도달',defenseChoiceButtons(mode,action,'reach',[['','미입력'],['easy','여유 있게'],['effort','노력해서'],['not_reached','도달 못함']],action.reach))+(action.reach==='not_reached'?'<div class="defense-derived"><span>처리 결과</span><b>해당 없음 · 미도달 기회로 분석</b></div>':defenseFieldGroup('처리 결과',defenseChoiceButtons(mode,action,'result',[['','미입력'],['clean','깔끔한 처리'],['recovered','불안정 후 보완'],['failed','처리 실패']],action.result)))+defenseOptionalDetailsHtml(mode,action)+defenseJudgmentHtml(mode,action);
  if(action.type==='throw')return defenseFieldGroup('송구 목적지',defenseChoiceButtons(mode,action,'target',[['','미입력'],['1B','1루'],['2B','2루'],['3B','3루'],['HOME','홈'],['RELAY','중계'],['OTHER','기타']],action.target))+defenseFieldGroup('송구 품질',defenseChoiceButtons(mode,action,'quality',[['','미입력'],['accurate','정확'],['catchable','받을 수 있음'],['uncatchable','받기 불가']],action.quality))+(action.quality&&action.quality!=='accurate'?defenseFieldGroup('벗어난 방향',defenseChoiceButtons(mode,action,'missDirection',[['','미입력'],['high','높음'],['low','낮음'],['left','왼쪽'],['right','오른쪽'],['bounce','바운드']],action.missDirection)):'')+defenseFieldGroup('송구 타이밍',defenseChoiceButtons(mode,action,'timing',[['','미입력'],['on_time','제시간'],['late','늦음'],['no_chance','아웃 기회 없음']],action.timing))+defenseFieldGroup('송구 부하',defenseChoiceButtons(mode,action,'tlu',[['0.75','가벼움','0.75 TLU'],['0.85','중간','0.85 TLU'],['1','전력','1.00 TLU']],String(action.tlu)))+defenseOptionalDetailsHtml(mode,action)+defenseJudgmentHtml(mode,action);
  if(action.type==='receive')return defenseFieldGroup('송구를 받은 위치',defenseChoiceButtons(mode,action,'target',[['','미입력'],['1B','1루'],['2B','2루'],['3B','3루'],['HOME','홈'],['RELAY','중계'],['OTHER','기타']],action.target))+defenseFieldGroup('들어온 송구',defenseChoiceButtons(mode,action,'incoming',[['','미입력'],['on_target','정확'],['high','높음'],['low','낮음'],['wide','벗어남'],['bounce','바운드'],['uncatchable','받기 불가']],action.incoming))+(action.incoming==='uncatchable'?'<div class="defense-derived neutral"><span>수신 결과</span><b>평가 제외 · 받을 수 없는 송구</b></div>':defenseFieldGroup('수신 결과',defenseChoiceButtons(mode,action,'result',[['','미입력'],['clean','깔끔한 수신'],['recovered','어려운 공 보완'],['failed','수신 실패']],action.result)))+defenseOptionalDetailsHtml(mode,action)+defenseJudgmentHtml(mode,action);
  if(action.type==='tag')return defenseFieldGroup('태그 실행',defenseChoiceButtons(mode,action,'execution',[['','미입력'],['clean','정확한 태그'],['recovered','수습 후 태그'],['missed','태그 빗나감'],['dropped','태그 중 공 빠짐']],action.execution))+defenseFieldGroup('태그 타이밍',defenseChoiceButtons(mode,action,'timing',[['','미입력'],['early','여유'],['close','접전'],['late','늦음'],['na','평가 불가']],action.timing))+defenseFieldGroup('심판 판정',defenseChoiceButtons(mode,action,'call',[['','미입력'],['out','아웃'],['safe','세이프'],['no_call','판정 없음']],action.call))+defenseOptionalDetailsHtml(mode,action)+defenseJudgmentHtml(mode,action);
  if(action.type==='base')return defenseFieldGroup('터치할 베이스',defenseChoiceButtons(mode,action,'base',[['','미입력'],['1B','1루'],['2B','2루'],['3B','3루'],['HOME','홈']],action.base))+defenseFieldGroup('베이스 접촉',defenseChoiceButtons(mode,action,'execution',[['','미입력'],['secure','정상 접촉'],['off_base','접촉 후 이탈'],['missed','미접촉']],action.execution))+defenseFieldGroup('베이스 타이밍',defenseChoiceButtons(mode,action,'timing',[['','미입력'],['early','여유'],['close','접전'],['late','늦음'],['na','평가 불가']],action.timing))+defenseFieldGroup('심판 판정',defenseChoiceButtons(mode,action,'call',[['','미입력'],['out','아웃'],['safe','세이프'],['no_call','판정 없음']],action.call))+defenseOptionalDetailsHtml(mode,action)+defenseJudgmentHtml(mode,action);
  return defenseFieldGroup('수비 역할',defenseChoiceButtons(mode,action,'role',[['','미입력'],['base_cover','베이스 커버'],['backup','백업'],['cutoff','중계 위치'],['communication','콜·의사소통']],action.role))+defenseFieldGroup('도착 시점',defenseChoiceButtons(mode,action,'timing',[['','미입력'],['on_time','제시간'],['late','늦음'],['missed','놓침']],action.timing))+defenseFieldGroup('수행 결과',defenseChoiceButtons(mode,action,'result',[['','미입력'],['correct','정확히 수행'],['recovered','늦었지만 보완'],['failed','수행 실패']],action.result))+defenseOptionalDetailsHtml(mode,action)+defenseJudgmentHtml(mode,action);
}
...
function defenseSituationHtml(mode,draft){
  const state=defenseEditorState[mode],open=state.detailsOpen.has('situation'),runnerOptions=[['1B','1루'],['2B','2루'],['3B','3루']];
  return `<details class="defense-situation" data-defense-details-toggle="situation" ${open?'open':''}><summary><span>플레이 상황 · 선택</span><small>판단 평가의 근거</small><i>⌄</i></summary><div class="defense-optional-body">${defenseFieldGroup('아웃카운트',defenseChoiceButtons(mode,null,'situation.outs',[['','미입력'],['0','0아웃'],['1','1아웃'],['2','2아웃']],draft.situation?.outs,{play:true}))}<div class="defense-field-group"><span>주자 상황</span><div class="defense-choice-grid runners">${runnerOptions.map(([value,label])=>{const selected=draft.situation?.runners?.includes(value);return `<button type="button" class="defense-choice ${selected?'active':''}" data-defense-runner="${value}" aria-pressed="${selected}"><b>${label}</b></button>`;}).join('')}</div></div></div></details>`;
}
...
function defenseOutcomeHtml(mode,draft){
  ...
  const missing=defenseMissingFields(draft),warnings=defenseFlowWarnings(draft),directOut=defenseDirectOutLabel(draft),official=defenseOfficialText(draft),recommendation=defenseOfficialRecommendation(draft),officialOpen=defenseEditorState[mode].detailsOpen.has('official');
  ...
  const outFields=draft.outcome==='out'?`${directOut?`<div class="defense-derived out-summary"><span>우리 선수의 직접 아웃</span><b>${esc(directOut)}</b></div>`:''}${defenseFieldGroup(directOut?'대표 아웃 방식 · 선택':'아웃 방식',defenseChoiceButtons(mode,null,'outMethod',[["",directOut?'동작별 판정 사용':'미입력'],['catch','뜬공 포구'],['tag','태그'],['base','베이스 터치'],['throw','송구에 의한 아웃']],draft.outMethod,{play:true}))}${defenseFieldGroup('기록한 아웃 수',defenseChoiceButtons(mode,null,'outsRecorded',[['','미입력'],['1','1개'],['2','2개'],['3','3개']],draft.outsRecorded,{play:true}))}`:'';
  const needsAttention=missing.length||warnings.length,attentionTitle=missing.length?`미입력 ${missing.length}개 · 기록에서 보완 가능`:warnings.length?`동작 연결 확인 ${warnings.length}개`:'필수 항목 입력 완료',attentionDetail=missing.length?missing.slice(0,3).map(x=>x.label).join(' · '):warnings.length?warnings.slice(0,2).map(x=>x.label).join(' · '):'판단 평가와 공식 기록은 선택 사항입니다.';
  return `<section class="defense-outcome"><div class="defense-section-title"><span>플레이 종료</span><small>각 동작 판정과 별도로 플레이 전체 결과를 기록합니다.</small></div>${defenseFieldGroup('플레이 결과',defenseChoiceButtons(mode,null,'outcome',[['','미입력'],['out','아웃'],['safe','세이프'],['continue','플레이 계속']],draft.outcome,{play:true}))}${outFields}<details class="defense-optional official" data-defense-details-toggle="official" ${officialOpen?'open':''}><summary><span>공식 기록 · 선택</span><small>${esc(official)}</small><i>⌄</i></summary><div class="defense-optional-body"><p class="defense-recommendation">${esc(recommendation)} · 추천은 자동 확정되지 않습니다.</p><div class="defense-official-state"><button type="button" data-defense-official-state="missing" class="${draft.official?.status==='missing'?'active':''}">미입력</button><button type="button" data-defense-official-state="none" class="${draft.official?.status==='none'?'active':''}">공식 기록 없음</button></div><div class="defense-official-grid">${officialButtons}</div></div></details><label class="defense-note-field">플레이 메모 · 선택<input data-defense-play-input="note" maxlength="500" value="${esc(draft.note||'')}" placeholder="영상 확인이나 코치 피드백" /></label><div class="defense-completeness ${needsAttention?'incomplete':'complete'}"><span aria-hidden="true">${needsAttention?'!':'✓'}</span><div><b>${esc(attentionTitle)}</b><small>${esc(attentionDetail)}</small></div></div><button type="button" class="defense-continue-play" data-defense-reopen-flow>＋ 플레이 동작 더 이어가기</button>${mode==='create'?'<button id="saveDefense" type="button" class="save-set defense-save">수비 플레이 저장</button>':''}</section>`;
}
...
function defenseTrainingActionCard(mode,action,index,total){const state=defenseTrainingEditorState[mode],open=state.openActionId===action.id,count=defenseTrainingActionCount(action,defenseTrainingDraft(mode).quantity);return `<section class="defense-step training-step ${open?'open':''}"><button type="button" class="defense-step-head" data-dt-toggle-action="${action.id}" aria-expanded="${open}"><span class="defense-step-number">${index+1}</span><span><b>${esc(defenseTrainingActionLabel(action.type))}</b><small>${esc(defenseTrainingActionSummary(action))}</small></span><em>${count}회</em><i>${open?'⌃':'⌄'}</i></button>${open?`<div class="defense-step-body"><label class="defense-action-kind">동작 종류<select data-dt-action-type data-dt-action-id="${action.id}">${[['field','타구 처리'],['receive','송구 받기'],['tag','주자 태그'],['base','베이스 터치'],['throw','송구'],['cover','커버·백업']].map(([value,label])=>`<option value="${value}" ${action.type===value?'selected':''}>${label}</option>`).join('')}</select></label>${defenseTrainingActionFields(mode,action)}<label class="training-action-count">실제 동작 횟수 · 선택<input type="number" min="0" data-dt-action-count data-dt-action-id="${action.id}" value="${action.countOverride??''}" placeholder="기본 ${defenseTrainingDraft(mode).quantity}회" /><small>비워 두면 세트 reps와 같은 ${defenseTrainingDraft(mode).quantity}회로 계산합니다.</small></label><div class="defense-step-tools"><button type="button" data-dt-move="up" data-dt-action-id="${action.id}" ${index===0?'disabled':''}>↑ 앞으로</button><button type="button" data-dt-move="down" data-dt-action-id="${action.id}" ${index===total-1?'disabled':''}>↓ 뒤로</button><button type="button" class="danger" data-dt-remove="${action.id}">이 동작 삭제</button></div><button type="button" class="defense-insert-action training-insert-action" data-dt-open-add="${index}">＋ 이 뒤에 동작 삽입</button></div>`:''}</section>`;}
function defenseTrainingScenarioHtml(mode,draft){const state=defenseTrainingEditorState[mode],stats=defenseTrainingStats(draft,draft.quantity),steps=draft.actions.map((action,index)=>`${defenseTrainingActionCard(mode,action,index,draft.actions.length)}${state.chooseAfter===index?defenseTrainingActionChooser(mode,index):''}`).join(''),chooser=!draft.actions.length?defenseTrainingActionChooser(mode,-1):'';return `<section class="defense-context-card training-scenario-context"><label>수비 포지션<select data-dt-field="position">${defenseTrainingPositionOptions(draft.position)}</select></label><label>영역<select data-dt-field="area"><option value="IF" ${draft.area==='IF'?'selected':''}>내야</option><option value="OF" ${draft.area==='OF'?'selected':''}>외야</option></select></label></section><div class="field-group"><span>시나리오 Reps</span>${defenseTrainingQuantityHtml(mode,draft)}</div><section class="training-flow-head"><div><span>SCENARIO FLOW</span><b>${draft.actions.length?draft.actions.map(action=>defenseTrainingActionShortLabel(action.type)).join(' → '):'동작을 추가하세요'}</b></div><small>동작 ${draft.actions.length}개</small></section><div class="defense-flow training-flow">${steps}${chooser}</div>${draft.actions.length&&state.chooseAfter===null?`<button type="button" class="defense-insert-action training-add-action" data-dt-open-add="${draft.actions.length-1}">＋ 다음 동작 이어가기</button>`:''}<div class="training-volume-preview"><div><span>세트 Reps</span><b>${draft.quantity}</b></div><div><span>Action Reps</span><b>${stats.actionReps}</b></div><div><span>Throws</span><b>${stats.throwCount}</b></div><div><span>Throw TLU</span><b>${n2(stats.throwTLU)}</b></div></div>`;}
...
function defenseTrainingEditorHtml(mode,{includeSave=false}={}){const draft=defenseTrainingDraft(mode);if(!draft)return '';return `${defenseTrainingModeTabs(mode,draft)}<p class="training-mode-help">${draft.trainingMode==='simple'?'훈련 종류와 총량을 빠르게 남깁니다. 기존 수비 훈련 기록도 이 방식으로 표시됩니다.':'실제 훈련 순서대로 동작을 구성하고 각 동작의 반복량과 송구 부하를 계산합니다.'}</p>${draft.trainingMode==='scenario'?defenseTrainingScenarioHtml(mode,draft):defenseTrainingSimpleHtml(mode,draft)}${defenseTrainingOutcomeHtml(mode,draft)}<label class="defense-note-field training-note-field">훈련 메모 · 선택<input data-dt-field="note" value="${esc(draft.note||'')}" placeholder="훈련 목표나 다음 세트 보완점" /></label>${includeSave?'<button class="save-set defense-training-save" type="button" data-save-training="defense">수비 훈련 저장</button>':''}`;}
...
function defenseValueLabel(group,value){const maps={difficulty:{routine:'쉬움',normal:'보통',difficult:'어려움'},reach:{easy:'여유 있게',effort:'노력해서',not_reached:'도달 못함'},fieldResult:{clean:'깔끔한 처리',recovered:'불안정 후 보완',failed:'처리 실패'},throwQuality:{accurate:'정확',catchable:'받을 수 있음',uncatchable:'받기 불가'},throwTiming:{on_time:'제시간',late:'늦음',no_chance:'아웃 기회 없음'},missDirection:{high:'높음',low:'낮음',left:'왼쪽',right:'오른쪽',bounce:'바운드'},incoming:{on_target:'정확',high:'높음',low:'낮음',wide:'벗어남',bounce:'바운드',uncatchable:'받기 불가'},receiveResult:{clean:'깔끔한 수신',recovered:'어려운 공 보완',failed:'수신 실패',excluded:'평가 제외'},technique:{normal:'일반',stretch:'스트레치',scoop:'스쿱',tag:'태그 · 기존 방식',block:'블로킹',base_hold:'베이스 유지 · 기존 방식'},tagExecution:{clean:'정확한 태그',recovered:'수습 후 태그',missed:'태그 빗나감',dropped:'태그 중 공 빠짐'},baseExecution:{secure:'정상 접촉',off_base:'접촉 후 이탈',missed:'미접촉'},outTiming:{early:'여유',close:'접전',late:'늦음',na:'평가 불가'},call:{out:'아웃',safe:'세이프',no_call:'판정 없음'},targetRunner:{BR:'타자주자',R1:'1루 주자',R2:'2루 주자',R3:'3루 주자',UNKNOWN:'대상 모름'},basePurpose:{force:'포스 플레이',appeal:'어필 플레이',other:'기타'},coverRole:{base_cover:'베이스 커버',backup:'백업',cutoff:'중계 위치',communication:'콜·의사소통'},coverTiming:{on_time:'제시간',late:'늦음',missed:'놓침'},coverResult:{correct:'정확히 수행',recovered:'늦었지만 보완',failed:'수행 실패'},outMethod:{catch:'뜬공 포구',tag:'태그',base:'베이스 터치',throw:'송구에 의한 아웃'}};return maps[group]?.[value]||'미입력';}
...
function defenseActionDetailRows(action){
  if(action.type==='field')return [['타구',action.battedBall||'미입력'],['처리 난도',defenseValueLabel('difficulty',action.difficulty)],['타구 도달',defenseValueLabel('reach',action.reach)],['처리 결과',action.reach==='not_reached'?'해당 없음':defenseValueLabel('fieldResult',action.result)],['처리 형태',fieldTypeLabel(action.fieldingType)]];
  if(action.type==='throw'){const rows=[['목적지',throwTargetLabel(action.target)||action.target||'미입력'],['송구 품질',defenseValueLabel('throwQuality',action.quality)],['타이밍',defenseValueLabel('throwTiming',action.timing)],['송구 TLU',n2(action.tlu||0)]];if(action.missDirection)rows.push(['벗어난 방향',defenseValueLabel('missDirection',action.missDirection)]);return rows;}
  if(action.type==='receive')return [['받은 위치',throwTargetLabel(action.target)||action.target||'미입력'],['들어온 송구',defenseValueLabel('incoming',action.incoming)],['수신 결과',defenseValueLabel('receiveResult',action.result)],['받는 기술',defenseValueLabel('technique',action.technique)]];
  ...
  if(action.type==='base')return [['터치 베이스',throwTargetLabel(action.base)||action.base||'미입력'],['플레이 목적',defenseValueLabel('basePurpose',action.purpose)],['베이스 접촉',defenseValueLabel('baseExecution',action.execution)],['타이밍',defenseValueLabel('outTiming',action.timing)],['심판 판정',defenseValueLabel('call',action.call)]];
...
function defenseCardHtml(e){
  const draft=normalizeDefenseMetadata(e.metadata||{}),expanded=expandedDefense.has(e.id),statuses=defenseCardStatuses(draft),tone=defenseOverallTone(draft),missing=draft.legacy?0:defenseMissingFields(draft).length,flow=draft.actions.map(action=>defenseActionShortLabel(action.type)).join(' → ')||'동작 미입력',headline=`${draft.position||'포지션 미입력'} · ${flow}`,meta=[defenseOutcomeText(draft),defenseOfficialText(draft)];if(draft.legacy)meta.push('기존 형식');else if(missing)meta.push(`미입력 ${missing}`);else meta.push(`${draft.actions.length}동작`);
  const commonRows=[['포지션',draft.position||'미입력'],['플레이 결과',defenseOutcomeText(draft)],['아웃 방식',draft.outcome==='out'?(draft.outMethod?defenseValueLabel('outMethod',draft.outMethod):defenseDirectOutLabel(draft)||'미입력'):'해당 없음'],['아웃 수',draft.outcome==='out'&&draft.outsRecorded?`${draft.outsRecorded}개`:'해당 없음'],['공식 기록',defenseOfficialText(draft)],['송구 TLU',n2(defenseThrowTLU(draft))]],judgment=defenseJudgmentSummary(draft);
  return `<article class="parent-card defense-card field-${tone}"><div class="parent-card-head"><button type="button" class="parent-toggle" data-toggle-record="defense:${e.id}" aria-expanded="${expanded}"><span class="defense-result-stack">${statuses.map(status=>`<b class="defense-result ${status.tone}">${esc(status.label)}</b>`).join('')}</span><span class="parent-ident"><b>${esc(headline)}</b><small>${esc(meta.join(' · '))}</small></span><span class="chev">${expanded?'⌃':'⌄'}</span></button></div>${expanded?`<div class="parent-card-body">${draft.legacy?'<p class="defense-legacy-note">기존 수비 기록입니다. 수정하면 새 순차 동작 형식으로 안전하게 전환됩니다.</p>':''}${defenseRecordTimelineHtml(draft)}<section class="defense-record-result"><h4>플레이 결과</h4><div class="defense-detail-grid">${commonRows.map(([label,value])=>`<div><span>${esc(label)}</span><b>${esc(value)}</b></div>`).join('')}</div>${judgment.evaluated?`<p class="defense-judgment-summary">판단 평가 ${judgment.evaluated}건 · 적절 ${judgment.appropriate} · 잘못된 판단 ${judgment.wrong}</p>`:''}${draft.note?`<p class="defense-note">${esc(draft.note)}</p>`:''}</section><div class="parent-card-foot actions-only">${eventRecordActionsHtml(e,'수비')}</div></div>`:''}</article>`;
}
...
function historyDefenseEvents(date){
  ...
  return recordsFor('gameEvents').filter(e=>{
    if(e.activityDate!==date||e.domain!=='defense')return false;const draft=normalizeDefenseMetadata(e.metadata||{}),missing=draft.legacy?0:defenseMissingFields(draft).length;
    if(ui.historyDefenseStatus==='complete'&&(draft.legacy||missing))return false;if(ui.historyDefenseStatus==='incomplete'&&(draft.legacy||!missing))return false;if(ui.historyDefenseStatus==='official_missing'&&draft.official.status!=='missing')return false;if(ui.historyDefenseStatus==='legacy'&&!draft.legacy)return false;
    if(ui.historyDefenseAction!=='all'&&!draft.actions.some(action=>action.type===ui.historyDefenseAction))return false;
    if(ui.historyFieldResult!=='all'&&!draft.actions.some(action=>action.type==='field'&&(ui.historyFieldResult==='not_reached'?action.reach==='not_reached':ui.historyFieldResult==='missing'?!action.result&&action.reach!=='not_reached':action.result===ui.historyFieldResult)))return false;
    if(ui.historyThrowResult!=='all'&&!draft.actions.some(action=>action.type==='throw'&&(ui.historyThrowResult==='missing'?!action.quality:action.quality===ui.historyThrowResult)))return false;
...
function renderHistoryConditions(){
  ...
  if(ui.historyMode!=='training'&&ui.historyDomain==='defense'){
    ...
    fields.push(historyFilterGroup('타구 처리','historyFieldResult',[['all','전체'],['clean','성공'],['recovered','보완'],['failed','실패'],['not_reached','미도달'],['missing','미입력']]));
    fields.push(historyFilterGroup('송구 품질','historyThrowResult',[['all','전체'],['accurate','정확'],['catchable','받기 가능'],['uncatchable','받기 불가'],['missing','미입력']]));
...
const ANALYSIS_METRICS={
  ...
  defense:{
    ...
    reachPct:{name:'Reach%',ko:'타구 도달률',full:'Batted-ball Reach Percentage',desc:'도달 여부가 평가된 타구 중 공에 도달한 비율입니다. 공식 지표가 아닌 자체 과정 지표입니다.',formula:'Reached / Evaluated reach opportunities',format:'pct'},
    ...
    onTimePct:{name:'On-Time%',ko:'제시간 송구율',full:'On-time Throw Percentage',desc:'아웃 기회가 있었던 송구 중 제시간에 보낸 비율입니다.',formula:'On-time / (On-time + Late)',format:'pct'},
...
function analysisMetricGroups(snapshot){
  if(ui.analysisSource==='game'){
    ...
    if(ui.analysisDomain==='defense')return [
      ...
      {title:'타구 처리 · 자체 지표',ids:['plays','reachPct','cleanPct','securePct']},
      {title:'송구 · 자체 지표',ids:['throwAttempts','onTargetPct','catchablePct','onTimePct','throwTLU']},
...
function renderAnalysisBreakdown(snapshot,range){
  ...
  if(ui.analysisSource==='game'&&ui.analysisDomain==='pitching')html=breakdownHtml('데이터 상태',{'완료 BF':s.bf,'강판 중단':s.relievedBF||0,'결과 미상':s.unknownBF,'미완료':s.incompleteBF,'투구 방향 미입력 공':s.missingPitcherSide||0,'상대 타자 미입력 공':s.missingBatterSide||0})+breakdownBarsHtml('타구 형태',s.battedTypes,{GB:'GB · 땅볼',LD:'LD · 라인드라이브',FB:'FB · 뜬공'})+breakdownBarsHtml('타구 방향',s.directions,{L:'좌',C:'중',R:'우'})+breakdownHtml('타구 결과',s.battedResults)+pitchingSplitTable(range);
    ...
    html=breakdownHtml('데이터 상태',{'입력 완료':s.dataStatus.complete,'미입력 있음':s.dataStatus.incomplete,'기존 형식':s.dataStatus.legacy,'공식 기록 미입력':s.official.missing})
      ...
      +breakdownBarsHtml('타구 도달',s.reach,{easy:'여유 있게',effort:'노력해서',not_reached:'도달 못함',missing:'미입력'})
      +breakdownBarsHtml('송구 품질',s.throwQuality,{accurate:'정확',catchable:'받을 수 있음',uncatchable:'받기 불가',missing:'미입력'})
      ...
      +breakdownBarsHtml('내야 처리 형태',s.ifTypes,{FRONT:'정면',FOREHAND:'포핸드',BACKHAND:'백핸드',CHARGE:'전진'})
      +breakdownBarsHtml('외야 접근',s.ofTypes,{FORWARD:'앞으로',STRAIGHT:'정면',LATERAL:'좌우',BACK:'뒤로'})
      +defenseThrowTargetTable(s.targetStats)+defenseFieldTypeThrowTable(s.fieldTypeThrowStats);
  }
...
function handleDefenseTrainingClick(event,mode){
  ...
  if(event.target.closest('[data-dt-cancel-add]')){defenseTrainingEditorState[mode].chooseAfter=null;renderDefenseTrainingEditor(mode);return;}
  const remove=event.target.closest('[data-dt-remove]');if(remove){const index=draft.actions.findIndex(action=>action.id===remove.dataset.dtRemove);if(index<0)return;const action=draft.actions[index];if(defenseTrainingActionHasData(action)&&!confirm(`${index+1}단계 ${defenseTrainingActionLabel(action.type)}와 입력 내용을 삭제할까요?`))return;draft.actions.splice(index,1);defenseTrainingEditorState[mode].openActionId=draft.actions[Math.min(index,draft.actions.length-1)]?.id||null;defenseTrainingEditorState[mode].chooseAfter=null;markDefenseTrainingDirty(mode);renderDefenseTrainingEditor(mode);return;}
...
function normalizeDefenseActionDependency(action,key,previous){
  if(action.type==='field'&&key==='reach'&&action.reach==='not_reached')action.result=null;
  if(action.type==='throw'&&key==='quality'&&action.quality==='accurate')action.missDirection=null;
...
function setDefenseActionValue(mode,action,key,value){const previous=action[key];action[key]=key==='tlu'?Number(value)||.85:(value||null);normalizeDefenseActionDependency(action,key,previous);markDefenseDraftDirty(mode);renderDefenseEditor(mode);}
...
function handleDefenseEditorClick(event,mode){
  ...
  if(event.target.closest('[data-defense-cancel-next]')){defenseEditorState[mode].chooseAfter=null;renderDefenseEditor(mode);return;}
  ...
  if(event.target.closest('[data-defense-reopen-flow]')){draft.flowEnded=false;draft.outcome=null;draft.outMethod=null;draft.outsRecorded=null;defenseEditorState[mode].chooseAfter=Math.max(-1,draft.actions.length-1);markDefenseDraftDirty(mode);renderDefenseEditor(mode);return;}
  const remove=event.target.closest('[data-defense-remove-action]');if(remove){const index=draft.actions.findIndex(action=>action.id===remove.dataset.defenseRemoveAction);if(index<0)return;const action=draft.actions[index];if(defenseActionHasUserData(action)&&!confirm(`${index+1}단계 ${defenseActionLabel(action.type)}와 입력 내용을 삭제할까요?`))return;draft.actions.splice(index,1);if(!draft.actions.length){draft.flowEnded=false;draft.outcome=null;draft.outMethod=null;draft.outsRecorded=null;}defenseEditorState[mode].openActionId=draft.actions[Math.min(index,draft.actions.length-1)]?.id||null;markDefenseDraftDirty(mode);renderDefenseEditor(mode);return;}
  ...
  const playButton=event.target.closest('[data-defense-play-key]');if(playButton){const key=playButton.dataset.defensePlayKey,value=playButton.dataset.defenseValue||null;if(key==='situation.outs')draft.situation.outs=value==null?null:Number(value);else{draft[key]=key==='outsRecorded'&&value?Number(value):value;if(key==='outcome'&&value!=='out'){draft.outMethod=null;draft.outsRecorded=null;}}markDefenseDraftDirty(mode);renderDefenseEditor(mode);return;}
  const runner=event.target.closest('[data-defense-runner]');if(runner){draft.situation=draft.situation||{outs:null,runners:[]};const value=runner.dataset.defenseRunner,set=new Set(draft.situation.runners||[]);set.has(value)?set.delete(value):set.add(value);draft.situation.runners=[...set];markDefenseDraftDirty(mode);renderDefenseEditor(mode);return;}
...
function handleDefenseEditorChange(event,mode){
  ...
  if(target.matches('[data-defense-play-input="position"]')){draft.position=target.value||'SS';for(const action of draft.actions.filter(x=>x.type==='field')){const allowed=new Set(defenseFieldDetailOptions(draft.position).map(x=>x[0]));if(!allowed.has(action.fieldingType||''))action.fieldingType=null;}markDefenseDraftDirty(mode);renderDefenseEditor(mode);return;}
...
async function saveDefensePlay(){
  const draft=ensureDefenseCreateDraft();if(!draft.actions.length){showToast('수비 동작을 선택하세요','우리 선수가 직접 수행한 첫 동작을 선택하세요.');return;}if(!draft.flowEnded){showToast('플레이를 종료하세요','마지막 동작에서 플레이 종료를 선택한 뒤 저장할 수 있습니다.');return;}
...
function openRecordEdit(store,id){const rec=data[store]?.find(x=>x.id===id);if(!rec)return;if(store==='trainingSets'){openTrainingEdit(id);return;}if(store==='gameEvents'&&rec.domain==='defense'){openDefenseEdit(id);return;}$('#editRecordStore').value=store;$('#editRecordId').value=id;let html=`<label>활동 날짜<input name="activityDate" type="date" value="${rec.activityDate||todayKey()}" /></label>`;
...
async function buildBackupPayload(){
  ...
  return {version:7,schemaVersion:7,appVersion:'7.7.0',exportOwnerId:uid,exportEmailHint:maskEmail(cloud.session?.user?.email||''),exportedAt:iso(),data:accountData};
}
