...
function defenseEditorHtml(mode){
  ...
  return `<section class="defense-context-card"><label>수비 포지션<select data-defense-play-input="position">${defensePositionOptions(draft.position)}</select></label><p><b>${esc(athlete()?.name||'선수')}</b>의 동작만 기록합니다. 동료의 포구 결과는 우리 선수의 송구 품질과 분리됩니다.</p></section><div class="defense-flow">${steps||defenseActionChooserHtml(mode,-1)}</div>${defenseOutcomeHtml(mode,draft)}`;
}
...
function handleDefenseEditorClick(event,mode){
  ...
  const playButton=event.target.closest('[data-defense-play-key]');if(playButton){const key=playButton.dataset.defensePlayKey,value=playButton.dataset.defenseValue||null;draft[key]=key==='outsRecorded'&&value!==null?Number(value):value;markDefenseDraftDirty(mode);renderDefenseEditor(mode);return;}
...
async function buildBackupPayload(){
  ...
  return {version:7,schemaVersion:7,appVersion:'7.8.0',exportOwnerId:uid,exportEmailHint:maskEmail(cloud.session?.user?.email||''),exportedAt:iso(),data:accountData};
}
