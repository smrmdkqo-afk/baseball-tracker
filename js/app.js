...
function defenseSituationHtml(mode,draft){
  const state=defenseEditorState[mode],open=state.detailsOpen.has('situation'),runnerOptions=[['1B','1루'],['2B','2루'],['3B','3루']],runners=Array.isArray(draft.situation?.runners)?draft.situation.runners:null;
  const situationSummary=draft.situation?.outs===null&&runners===null?'미입력':`${draft.situation?.outs===null?'아웃 미입력':`${draft.situation.outs}아웃`} · ${runners===null?'주자 미입력':runners.length?runners.map(x=>({'1B':'1루','2B':'2루','3B':'3루'}[x])).join('·'):'주자 없음'}`;
  return `<details class="defense-situation" data-defense-details-toggle="situation" ${open?'open':''}><summary><span>플레이 시작 상황 · 선택</span><small>${situationSummary}</small><i>⌄</i></summary><div class="defense-optional-body">${defenseFieldGroup('플레이 시작 전 아웃 카운트',defenseChoiceButtons(mode,null,'situation.outs',[['','미입력'],['0','0아웃'],['1','1아웃'],['2','2아웃']],draft.situation?.outs,{play:true}))}<div class="defense-field-group"><span>플레이 시작 전 주자 상황</span><div class="defense-choice-grid runners before-play"><button type="button" class="defense-choice ${runners===null?'active':''}" data-defense-runners-before-missing aria-pressed="${runners===null}"><b>미입력</b></button><button type="button" class="defense-choice ${runners?.length===0?'active':''}" data-defense-runners-before-none aria-pressed="${runners?.length===0}"><b>주자 없음</b></button>${runnerOptions.map(([value,label])=>{const selected=runners?.includes(value);return `<button type="button" class="defense-choice ${selected?'active':''}" data-defense-runner-before="${value}" aria-pressed="${!!selected}"><b>${label}</b></button>`;}).join('')}</div></div><p class="defense-helper">상황별 수비 분석에만 사용하며, 선택하지 않아도 기록을 저장할 수 있습니다.</p></div></details>`;
}
...
async function buildBackupPayload(){
  ...
  return {version:7,schemaVersion:7,appVersion:'7.8.1',exportOwnerId:uid,exportEmailHint:maskEmail(cloud.session?.user?.email||''),exportedAt:iso(),data:accountData};
}
