import {configureAccountDB,closeDB,deleteAccountDatabase,getAll,putOne,putMany,getMeta,setMeta,snapshot,replaceSnapshot,mergeSnapshot,ensureInitialData,inspectLegacyData,snapshotOwnerIds,previewSnapshot,uuid,iso,todayKey,stamp} from './storage.js?v=7.4.2';
import {gamePitchingSummary,battingSummary,defenseSummary,baserunningSummary,trainingSummary,workloadSummary,todaySummary,totalTLU,analysisSnapshot,analysisMetricValue,analysisSeries,localDate,dateShift,OFFICIAL_PITCH_TYPES,STRIKE_PITCH_TYPES,GAME_TLU,round2,canonicalGameEvents,gameEventIntegrity} from './analytics.js?v=7.4.2';
import {anchoredAnalysisRange,activityDateNavigation,activityDatesInRange} from './analysis-scope.js?v=7.4.2';
...
function openRecordEdit(store,id){const rec=data[store]?.find(x=>x.id===id);if(!rec)return;$('#editRecordStore').value=store;$('#editRecordId').value=id;let html=`<label>활동 날짜<input name="activityDate" type="date" value="${rec.activityDate||todayKey()}" /></label>`;
  if(store==='batterFaced')html+=`<label>전체 투구 방향<select name="pitcherSide"><option value="">미입력</option><option value="R" ${rec.pitcherSide==='R'?'selected':''}>우투</option><option value="L" ${rec.pitcherSide==='L'?'selected':''}>좌투</option></select></label><label>전체 상대 타자<select name="batterSide"><option value="">미입력</option><option value="R" ${rec.batterSide==='R'?'selected':''}>우타</option><option value="L" ${rec.batterSide==='L'?'selected':''}>좌타</option></select></label><p class="parent-edit-scope span-2">방향을 바꾸면 이 타자에게 기록한 모든 공에 일괄 적용됩니다. 한 공만 고치려면 펼친 기록에서 해당 공을 선택하세요.</p><div class="derived-field"><span>결과</span><b>${esc(parentResultLabel(rec))}</b><small>투구 기록에서 자동 계산</small></div>`;
    ...
    if(rec.domain==='defense')html+=`<label>포지션<select name="position">${['SS','2B','3B','1B','C','P','LF','CF','RF'].map(v=>`<option ${m.position===v?'selected':''}>${v}</option>`).join('')}</select></label><label>포구 결과<select name="fieldingResult">${[['success','성공'],['unstable','불안정'],['failed','실패']].map(([v,l])=>`<option value="${v}" ${m.fieldingResult===v?'selected':''}>${l}</option>`).join('')}</select></label><label>포구 형태<select name="fieldingType"><option value="">미입력</option>${[['FRONT','정면'],['FOREHAND','포핸드'],['BACKHAND','백핸드'],['CHARGE','전진'],['FORWARD','앞으로'],['STRAIGHT','정면(외야)'],['LATERAL','좌우'],['BACK','뒤로']].map(([v,l])=>`<option value="${v}" ${m.fieldingType===v?'selected':''}>${l}</option>`).join('')}</select></label><label>송구<select name="throwResult">${[['none','없음'],['success','정상 송구'],['error','악송구']].map(([v,l])=>`<option value="${v}" ${m.throwResult===v?'selected':''}>${l}</option>`).join('')}</select></label><label>송구 목적지<select name="throwTarget"><option value="">미입력</option>${[['1B','1루'],['2B','2루'],['3B','3루'],['HOME','홈'],['RELAY','중계']].map(([v,l])=>`<option value="${v}" ${m.throwTarget===v?'selected':''}>${l}</option>`).join('')}</select></label><label>송구 부하<select name="throwTLU">${[['0.75','0.75'],['0.85','0.85'],['1','1.00']].map(([v,l])=>`<option value="${v}" ${Number(m.throwTLU)===Number(v)?'selected':''}>${l}</option>`).join('')}</select></label>`;
...
async function buildBackupPayload(){
  ...
  return {version:7,schemaVersion:7,appVersion:'7.4.2',exportOwnerId:uid,exportEmailHint:maskEmail(cloud.session?.user?.email||''),exportedAt:iso(),data:accountData};
}
