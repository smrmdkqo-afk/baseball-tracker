...
function latestRecordedSituation(date=ui.inputDate){
  const latest=recordsFor('gameEvents').filter(event=>event.activityDate===date&&Object.prototype.hasOwnProperty.call(event.metadata||{},'situation')).sort((a,b)=>new Date(b.recordedAt)-new Date(a.recordedAt))[0];
  return latest?cloneGameSituation(latest.metadata.situation):{outs:null,runners:null};
}
...
function matchupGameSituationHtml(){return `<div class="matchup-situation"><div class="game-situation-head"><span>현재 경기 상황</span><small>다음 공에 적용</small></div>${gameSituationFieldsHtml(currentGameSituation())}</div>`;}
...
function sideButtonsPitching(bf){const matchup=currentPitchingMatchup(bf);return `<section class="matchup-context" aria-label="다음 공의 경기 조건"><div class="matchup-context-head"><span>현재 투타 조건</span><small>선택한 조건은 다음 공부터 적용됩니다</small></div><div class="matchup-grid"><div class="field-group"><span>투구 방향</span><div class="side-context">${sideBtn('pitcher','R','우투',matchup.pitcherSide)}${sideBtn('pitcher','L','좌투',matchup.pitcherSide)}</div></div><div class="field-group"><span>상대 타자</span><div class="side-context">${sideBtn('batter','','미입력',matchup.batterSide)}${sideBtn('batter','R','우타',matchup.batterSide)}${sideBtn('batter','L','좌타',matchup.batterSide)}</div></div></div>${matchupGameSituationHtml()}</section>`;}
...
async function recordGameThrow(type){const gd=await ensureGameDay(),throwSide=currentPitchingMatchup().pitcherSide,situation=cloneGameSituation(currentGameSituation());const e={id:uuid(),athleteId:activeAthleteId,gameDayId:gd.id,domain:'pitching',parentType:null,parentId:null,eventType:type,activityDate:ui.inputDate,recordedAt:iso(),metadata:{tlu:GAME_TLU[type],throwSide,situation},ownerId:accountOwnerId(),deletedAt:null};await save('gameEvents',e);}
...
async function buildBackupPayload(){
  ...
  return {version:7,schemaVersion:7,appVersion:'7.9.0',exportOwnerId:uid,exportEmailHint:maskEmail(cloud.session?.user?.email||''),exportedAt:iso(),data:accountData};
}
