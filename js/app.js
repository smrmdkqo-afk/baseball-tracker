...
function openRecordEdit(store,id){const rec=data[store]?.find(x=>x.id===id);if(!rec)return;if(store==='gameEvents'&&rec.domain==='defense'){openDefenseEdit(id);return;}$('#editRecordStore').value=store;$('#editRecordId').value=id;let html=`<label>활동 날짜<input name="activityDate" type="date" value="${rec.activityDate||todayKey()}" /></label>`;
  if(store==='batterFaced')html+=`<label>전체 투구 방향<select name="pitcherSide"><option value="">미입력</option><option value="R" ${rec.pitcherSide==='R'?'selected':''}>우투</option><option value="L" ${rec.pitcherSide==='L'?'selected':''}>좌투</option></select></label><label>전체 상대 타자<select name="batterSide"><option value="">미입력</option><option value="R" ${rec.batterSide==='R'?'selected':''}>우타</option><option value="L" ${rec.batterSide==='L'?'selected':''}>좌타</option></select></label><p class="parent-edit-scope span-2">방향을 바꾸면 이 타자에게 기록한 모든 공에 일괄 적용됩니다. 한 공만 고치려면 펼친 기록에서 해당 공을 선택하세요.</p><div class="derived-field"><span>결과</span><b>${esc(parentResultLabel(rec))}</b><small>투구 기록에서 자동 계산</small></div>`;
    ...
    if(rec.domain==='defense')html+=`<label>실제 송구 횟수<input name="throwCount" type="number" min="0" value="${rec.metadata?.throwCount||0}" /></label><label>송구 TLU/회<select name="throwIntensity"><option value="0" ${Number(rec.metadata?.throwIntensity)===0&&rec.metadata?.throwIntensity!=null?'selected':''}>매우 가벼운 토스 · 0.00</option><option value="0.75" ${rec.metadata?.throwIntensity==null||Number(rec.metadata?.throwIntensity)===.75?'selected':''}>근거리 · 0.75</option><option value="0.85" ${Number(rec.metadata?.throwIntensity)===.85?'selected':''}>중거리 · 0.85</option><option value="1" ${Number(rec.metadata?.throwIntensity)===1?'selected':''}>장거리/전력 · 1.00</option></select></label>`;
  }
...
async function buildBackupPayload(){
  ...
  return {version:7,schemaVersion:7,appVersion:'7.6.1',exportOwnerId:uid,exportEmailHint:maskEmail(cloud.session?.user?.email||''),exportedAt:iso(),data:accountData};
}
