...
let ui={view:'home',inputDate:todayKey(),inputMode:'game',domain:'pitching',historyAnchor:todayKey(),historyPeriod:'all',historyMode:'all',historyDomain:'all',historyStatus:'all',historyOwnSide:'all',historyOppSide:'all',historyDefenseStatus:'all',historyDefenseAction:'all',historyFieldResult:'all',historyThrowResult:'all',historyReceiveResult:'all',historyOfficial:'all',historyRunResult:'all',analysisSource:'game',analysisAnchor:todayKey(),analysisPeriod:'1',analysisView:'game',analysisDomain:'pitching',analysisMetric:'strikePct',ownSide:'all',oppSide:'all',pendingBatterSide:null,pendingOwnPitchSide:null,pendingBatSide:null,pendingOppPitcherSide:null,inPlayContext:null,quantity:10,inputSummaryCollapsed:null};
...
function defenseActionChooserHtml(mode,after){
  const options=[['field','타구 처리','직접 타구를 처리'],['receive','송구 받기','동료의 송구를 받음'],['throw','송구','목표로 공을 보냄'],['cover','커버·백업','공 없이 수비에 참여']];
  return `<section class="defense-action-chooser"><div><span>${after<0?'첫 동작 선택':'다음 동작 선택'}</span><small>우리 선수가 직접 한 동작만 선택하세요.</small></div><div class="defense-action-type-grid">${options.map(([value,label,sub])=>`<button type="button" data-defense-add-type="${value}" data-defense-after="${after}"><span aria-hidden="true">${{field:'◆',receive:'⌒',throw:'➜',cover:'◇'}[value]}</span><b>${label}</b><small>${sub}</small></button>`).join('')}</div>${after>=0?`<button type="button" class="defense-cancel-next" data-defense-cancel-next>다음 동작 추가 취소</button>`:''}</section>`;
}
...
function defenseOptionalDetailsHtml(mode,action){
  ...
  else if(action.type==='receive')body=defenseFieldGroup('받는 기술 · 선택',defenseChoiceButtons(mode,action,'technique',[['','미입력'],['normal','일반'],['stretch','스트레치'],['scoop','스쿱'],['tag','태그'],['block','블로킹'],['base_hold','베이스 유지']],action.technique));
...
function defenseActionCardHtml(mode,action,index,total,previous=null){
  ...
  const typeSelect=`<label class="defense-action-kind">동작 종류<select data-defense-action-type-input data-defense-action-id="${action.id}">${[['field','타구 처리'],['receive','송구 받기'],['throw','송구'],['cover','커버·백업']].map(([value,label])=>`<option value="${value}" ${action.type===value?'selected':''}>${label}</option>`).join('')}</select></label>`;
...
function defenseOutcomeHtml(mode,draft){
  ...
  const missing=defenseMissingFields(draft),official=defenseOfficialText(draft),recommendation=defenseOfficialRecommendation(draft),officialOpen=defenseEditorState[mode].detailsOpen.has('official');
  ...
  return `<section class="defense-outcome"><div class="defense-section-title"><span>플레이 종료</span><small>동작과 별도로 실제 결과를 기록합니다.</small></div>${defenseFieldGroup('플레이 결과',defenseChoiceButtons(mode,null,'outcome',[['','미입력'],['out','아웃'],['safe','세이프'],['continue','플레이 계속']],draft.outcome,{play:true}))}${draft.outcome==='out'?defenseFieldGroup('아웃 방식',defenseChoiceButtons(mode,null,'outMethod',[['','미입력'],['catch','뜬공 포구'],['tag','태그'],['base','베이스 터치'],['throw','송구에 의한 아웃']],draft.outMethod,{play:true}))+defenseFieldGroup('기록한 아웃 수',defenseChoiceButtons(mode,null,'outsRecorded',[['','미입력'],['1','1개'],['2','2개'],['3','3개']],draft.outsRecorded,{play:true})):''}<details class="defense-optional official" data-defense-details-toggle="official" ${officialOpen?'open':''}><summary><span>공식 기록 · 선택</span><small>${esc(official)}</small><i>⌄</i></summary><div class="defense-optional-body"><p class="defense-recommendation">${esc(recommendation)} · 추천은 자동 확정되지 않습니다.</p><div class="defense-official-state"><button type="button" data-defense-official-state="missing" class="${draft.official?.status==='missing'?'active':''}">미입력</button><button type="button" data-defense-official-state="none" class="${draft.official?.status==='none'?'active':''}">공식 기록 없음</button></div><div class="defense-official-grid">${officialButtons}</div></div></details><label class="defense-note-field">플레이 메모 · 선택<input data-defense-play-input="note" maxlength="500" value="${esc(draft.note||'')}" placeholder="영상 확인이나 코치 피드백" /></label><div class="defense-completeness ${missing.length?'incomplete':'complete'}"><span aria-hidden="true">${missing.length?'!':'✓'}</span><div><b>${missing.length?`미입력 ${missing.length}개 · 기록에서 보완 가능`:'필수 항목 입력 완료'}</b><small>${missing.length?esc(missing.slice(0,3).map(x=>x.label).join(' · ')):'판단 평가와 공식 기록은 선택 사항입니다.'}</small></div></div><button type="button" class="defense-continue-play" data-defense-reopen-flow>＋ 플레이 동작 더 이어가기</button>${mode==='create'?'<button id="saveDefense" type="button" class="save-set defense-save">수비 플레이 저장</button>':''}</section>`;
}
function defenseEditorHtml(mode){
  const draft=defenseDraft(mode);if(!draft)return '';
  const state=defenseEditorState[mode],steps=draft.actions.map((action,index)=>defenseActionCardHtml(mode,action,index,draft.actions.length,draft.actions[index-1]||null)).join('');
...
function defenseJudgmentDetailRows(action){const j=action.judgment||{},rows=[];if(!j.rating)return rows;if(action.type==='field'){if(j.reaction)rows.push(['첫 반응',{good:'빠름',normal:'보통',late:'늦음',wrong:'반대 방향'}[j.reaction]]);if(j.route)rows.push(['이동 경로',{direct:'효율적',adjusted:'수정함',inefficient:'비효율',na:'해당 없음'}[j.route]]);}else if(action.type==='throw'){if(j.bestChoice)rows.push(['더 나은 선택',{hold:'던지지 않기','1B':'1루','2B':'2루','3B':'3루',HOME:'홈',RELAY:'중계'}[j.bestChoice]]);}else if(action.type==='receive'){if(j.positioning)rows.push(['위치 선정',{correct:'적절',late:'늦음',wrong:'잘못됨'}[j.positioning]]);if(j.nextReady)rows.push(['다음 플레이',{ready:'준비됨',late:'늦음',na:'해당 없음'}[j.nextReady]]);}else if(j.communication)rows.push(['의사소통',{good:'적절',late:'늦음',missed:'없었음',na:'해당 없음'}[j.communication]]);return rows.filter(([,value])=>value);}
...
function defenseCardHtml(e){
  ...
  const commonRows=[['포지션',draft.position||'미입력'],['플레이 결과',defenseOutcomeText(draft)],['아웃 방식',draft.outcome==='out'?defenseValueLabel('outMethod',draft.outMethod):'해당 없음'],['아웃 수',draft.outcome==='out'&&draft.outsRecorded?`${draft.outsRecorded}개`:'해당 없음'],['공식 기록',defenseOfficialText(draft)],['송구 TLU',n2(defenseThrowTLU(draft))]],judgment=defenseJudgmentSummary(draft);
...
function normalizeHistoryFilters(){
  ...
  if(ui.historyDomain!=='defense'||ui.historyMode==='training'){ui.historyDefenseStatus='all';ui.historyDefenseAction='all';ui.historyFieldResult='all';ui.historyThrowResult='all';ui.historyReceiveResult='all';ui.historyOfficial='all';}
...
function resetHistoryConditions({render=true}={}){ui.historyStatus='all';ui.historyOwnSide='all';ui.historyOppSide='all';ui.historyDefenseStatus='all';ui.historyDefenseAction='all';ui.historyFieldResult='all';ui.historyThrowResult='all';ui.historyReceiveResult='all';ui.historyOfficial='all';ui.historyRunResult='all';if(render)renderHistory();}
...
function historyConditionText(){
  ...
  const fieldLabels={clean:'처리 성공',recovered:'처리 보완',failed:'처리 실패',not_reached:'미도달',missing:'처리 미입력'},throwLabels={accurate:'송구 정확',catchable:'송구 가능',uncatchable:'송구 불가',missing:'송구 미입력'},receiveLabels={clean:'수신 성공',recovered:'수신 보완',failed:'수신 실패',excluded:'평가 제외',missing:'수신 미입력'},defenseStatusLabels={complete:'입력 완료',incomplete:'미입력 있음',official_missing:'공식 미입력',legacy:'기존 형식'},officialLabels={po:'공식 PO',a:'공식 A',e:'공식 E',dp:'공식 DP',none:'공식 기록 없음',missing:'공식 기록 미입력'};
  if(ui.historyDefenseStatus!=='all')values.push(defenseStatusLabels[ui.historyDefenseStatus]);if(ui.historyDefenseAction!=='all')values.push(defenseActionLabel(ui.historyDefenseAction));if(ui.historyFieldResult!=='all')values.push(fieldLabels[ui.historyFieldResult]);if(ui.historyThrowResult!=='all')values.push(throwLabels[ui.historyThrowResult]);if(ui.historyReceiveResult!=='all')values.push(receiveLabels[ui.historyReceiveResult]);if(ui.historyOfficial!=='all')values.push(officialLabels[ui.historyOfficial]);if(ui.historyRunResult!=='all')values.push(ui.historyRunResult==='SUCCESS'?'도루 성공':'도루 실패');return values.length?values.join(' · '):'전체 조건';
}
function renderHistoryConditions(){
  ...
  if(ui.historyMode!=='training'&&ui.historyDomain==='defense'){
    ...
    fields.push(historyFilterGroup('수비 동작','historyDefenseAction',[['all','전체'],['field','타구 처리'],['receive','송구 받기'],['throw','송구'],['cover','커버·백업']]));
...
function metricSample(snapshot){
  if(snapshot.source==='game'){
    ...
    if(snapshot.domain==='defense')return `${s.plays} Plays · ${s.fieldAttempts} Field chances · ${s.throwAttempts} Throws${s.dataStatus.incomplete?` · 미입력 ${s.dataStatus.incomplete}건`:''}${s.dataStatus.legacy?` · 기존 형식 ${s.dataStatus.legacy}건`:''}`;
...
function renderAnalysisBreakdown(snapshot,range){
  ...
  if(ui.analysisSource==='game'&&ui.analysisDomain==='pitching')html=breakdownHtml('데이터 상태',{'완료 BF':s.bf,'강판 중단':s.relievedBF||0,'결과 미상':s.unknownBF,'미완료':s.incompleteBF,'투구 방향 미입력 공':s.missingPitcherSide||0,'상대 타자 미입력 공':s.missingBatterSide||0})+breakdownBarsHtml('타구 형태',s.battedTypes,{GB:'GB · 땅볼',LD:'LD · 라인드라이브',FB:'FB · 뜬공'})+breakdownBarsHtml('타구 방향',s.directions,{L:'좌',C:'중',R:'우'})+breakdownHtml('타구 결과',s.battedResults)+pitchingSplitTable(range);
    ...
    html=breakdownHtml('데이터 상태',{'입력 완료':s.dataStatus.complete,'미입력 있음':s.dataStatus.incomplete,'기존 형식':s.dataStatus.legacy,'공식 기록 미입력':s.official.missing})
      +breakdownBarsHtml('수비 동작',s.actionCounts,{field:'타구 처리',throw:'송구',receive:'송구 받기',cover:'커버·백업'})
...
function bindStaticEvents(){
  ...
  $$('[data-close]').forEach(b=>b.addEventListener('click',()=>hideModal(b.dataset.close)));$$('[data-close-defense-edit]').forEach(b=>b.addEventListener('click',requestCloseDefenseEdit));document.querySelectorAll('.modal-backdrop').forEach(m=>m.addEventListener('click',e=>{if(e.target!==m)return;if(m.id==='defenseEditModal')requestCloseDefenseEdit();else m.hidden=true;}));
  $('#inPlayResults').addEventListener('click',e=>{const b=e.target.closest('[data-result]');if(b)completeInPlay(b.dataset.result);});
  $('#recordEditForm').addEventListener('submit',saveEditedRecord);$('#pitchEditForm').addEventListener('submit',savePitchEdit);$('#deletePitchBtn').addEventListener('click',deletePitchFromEdit);$('#defenseEditForm').addEventListener('submit',saveDefenseEdit);$('#defenseEditDate').addEventListener('change',()=>{defenseEditDirty=true;});
...
async function saveDefensePlay(){
  const draft=ensureDefenseCreateDraft();if(!draft.actions.length){showToast('수비 동작을 선택하세요','타구 처리, 송구 받기, 송구, 커버·백업 중 하나를 먼저 선택하세요.');return;}if(!draft.flowEnded){showToast('플레이를 종료하세요','마지막 동작에서 플레이 종료를 선택한 뒤 저장할 수 있습니다.');return;}
...
function openRecordEdit(store,id){const rec=data[store]?.find(x=>x.id===id);if(!rec)return;if(store==='gameEvents'&&rec.domain==='defense'){openDefenseEdit(id);return;}$('#editRecordStore').value=store;$('#editRecordId').value=id;let html=`<label>활동 날짜<input name="activityDate" type="date" value="${rec.activityDate||todayKey()}" /></label>`;
...
async function buildBackupPayload(){
  ...
  return {version:7,schemaVersion:7,appVersion:'7.5.0',exportOwnerId:uid,exportEmailHint:maskEmail(cloud.session?.user?.email||''),exportedAt:iso(),data:accountData};
}
...
function resetAccountMemory(){
  data={athletes:[],gameDays:[],batterFaced:[],plateAppearances:[],gameEvents:[],trainingSets:[]};activeAthleteId=null;resumeContext=null;expandedBF.clear();expandedPA.clear();expandedDefense.clear();expandedBaserunning.clear();defenseCreateDraft=null;defenseEditDraft=null;defenseEditId=null;defenseEditDirty=false;resetDefenseEditorState('create');resetDefenseEditorState('edit');lastDeleted=null;pendingRestore=null;ui.analysisAnchor=todayKey();ui.analysisPeriod='1';ui.ownSide='all';ui.oppSide='all';analysisConditionsInitialized=false;
